...
* User-visible changes:
  ...
  * This version introduces:
    ...
    * `plot_risk()`, a function that plots risk estimates generated by `estimate_risk()` / `est_risk()`

    * A new vignette "plot-risk" to accompany the new function

    * The ability to pass a data frame as the first argument of `estimate_risk()` / `est_risk()`
      (e.g., instead of `est_risk(use_dat = df)`, users can now do `est_risk(df)`, which will automatically
      translate to `est_risk(use_dat = df)` in the background; see the updated function documentation for
      more information)

    * Minor tweaks and additions to existing documentation

* Internal changes:

  * Rename "prevent_equations.R" to "estimate_risk.R"
  
  * Add test suite for `plot_risk()`

  * Correct typo for the `sysdata.rda` file for one of the coefficients;
    this is strictly typographical and has no impact on the calculations or functionality
    of the package (thanks to @kcjw87 for reporting this, #2)
